...
import {useGetState} from '../../api/tv-shows/use-get-states';
...
const Header = ({
  ...
  const handleFavourites = (value: boolean) => {
    favorite(
      {media_id, media_type: 'tv', favorite: value},
      {onSuccess: refetch},
    );
  };
  ...
  const handleWatchlist = (value: boolean) => {
    watchlist(
      {media_id, media_type: 'tv', watchlist: value},
      {onSuccess: refetch},
    );
  };
