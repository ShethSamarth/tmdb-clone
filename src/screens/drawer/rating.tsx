...
import {useGetRated as useGetRatedMovies} from '../../api/movies/use-get-rated';
import {useGetRated as useGetRatedTvShows} from '../../api/tv-shows/use-get-rated';
...
const Rating = inject('AuthStore')(
  observer(({AuthStore}: {AuthStore?: AuthStoreType}) => {
    ...
    } = useGetRatedMovies(AuthStore?.session_id!);
    const {
      data: tvShows,
      isLoading: tvShowsLoading,
      refetch: refetchTvShows,
    } = useGetRatedTvShows(AuthStore?.session_id!);
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          />
          <Text style={DYNAMIC.h4(FONTS.ibm, COLORS.primary)}>My Ratings</Text>
          <View />
        </View>
        ...
        {selectedTab === 0 ? (
          movies && movies.length > 0 ? (
            ...
            <Text style={styles.center}>No movies rated</Text>
          )
        ) : tvShows && tvShows.length > 0 ? (
          <TvVerticalList data={tvShows!} refetch={refetchTvShows} />
        ) : (
          <Text style={styles.center}>No tv shows rated</Text>
        )}
      </View>
    );
  }),
